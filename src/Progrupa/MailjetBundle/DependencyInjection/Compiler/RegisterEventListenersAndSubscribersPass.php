...
class RegisterEventListenersAndSubscribersPass implements CompilerPassInterface
{
    ...
    {
        $listeners = array();
        foreach ($container->findTaggedServiceIds('mailjet.serializer.event_listener') as $id => $tags) {
            if (!$container->getDefinition($id)->isPublic()) {
                throw new \RuntimeException(sprintf('The tag "mailjet.serializer.event_listener" of service "%s" requires the service to be public.', $id));
            }

            foreach ($tags as $attributes) {
                if ( ! isset($attributes['event'])) {
                    throw new \RuntimeException(sprintf('The tag "mailjet.serializer.event_listener" of service "%s" requires an attribute named "event".', $id));
                }
        ...
        foreach ($container->findTaggedServiceIds('mailjet.serializer.event_subscriber') as $id => $tags) {
        ...
        if ($listeners) {
            ...
            $container->getDefinition('mailjet.serializer.event_dispatcher')
