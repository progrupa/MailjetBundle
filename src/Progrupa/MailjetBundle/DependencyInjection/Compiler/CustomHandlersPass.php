...
namespace Progrupa\MailjetBundle\DependencyInjection\Compiler;
...
class CustomHandlersPass implements CompilerPassInterface
{
    ...
    {
        $handlers = array();
        foreach ($container->findTaggedServiceIds('mailjet.serializer.handler') as $id => $tags) {
            foreach ($tags as $attrs) {
                if ( ! isset($attrs['type'], $attrs['format'])) {
                    throw new \RuntimeException(sprintf('Each tag named "mailjet.serializer.handler" of service "%s" must have at least two attributes: "type" and "format".', $id));
                }
                ...
                if (isset($attrs['direction'])) {
                    if ( ! defined($directionConstant = 'JMS\Serializer\GraphNavigator::DIRECTION_'.strtoupper($attrs['direction']))) {
                        throw new \RuntimeException(sprintf('The direction "%s" of tag "mailjet.serializer.handler" of service "%s" does not exist.', $attrs['direction'], $id));
                    }
        ...
        foreach ($container->findTaggedServiceIds('mailjet.serializer.subscribing_handler') as $id => $tags) {
        ...
        $container->getDefinition('mailjet.serializer.handler_registry')
